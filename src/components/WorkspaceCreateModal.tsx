...
export default function WorkspaceCreateModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "워크스페이스 생성에 실패했습니다.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          <div>
            <div className="text-sm font-semibold text-gray-900">새 워크스페이스</div>
            <div className="text-xs text-gray-500">브랜드·프로젝트·팀 단위로 페이지를 분리합니다</div>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-4 py-4">
          ...
          <input
            ...
            placeholder="예: A브랜드 마케팅 / 학원 운영 / 개인"
